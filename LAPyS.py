...
LDAP_SEACRCH_BASE_DIR = <PATH TO OU>

def CheckNetwork():
    try:
        hostname, domain = socket.gethostbyaddr("<ENTER YOUR DC IP ADDRESS>")[0].partition('.')[::2]
...
def get_ldap_info(UserName, PasswordLocal, ComName):
    try:
        with Connection(Server("<ENTER YOUR DC IP ADDRESS>", port=389, use_ssl=False), auto_bind=AUTO_BIND_NO_TLS, user="<YOUR DOMAIN NAME>\\{0}".format(UserName), password=PasswordLocal) as c:
...
TextBoxDomainComputerName.bind("<Key>")
